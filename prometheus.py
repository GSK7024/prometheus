...
class LearningDatabase:
    ...
    def add_learning(self, operation, context, outcome, improvement_notes=None):
        """Add a successful learning experience"""
        learning = {
            'id': str(uuid.uuid4()),
            'operation': operation,
            'context': context,
    ...
    def add_failure(self, operation, context, error, solution_attempted=None, resolution=None):
        ...
        existing = self._find_similar_failure(failure)
        if existing:
            existing['frequency'] += 1
            existing['last_seen'] = time.time()
            if resolution and not existing.get('resolved'):
    ...
    def _find_similar_learning(self, learning):
        """Find similar learning experiences"""
        for l in self.learnings:
            if (l['operation'] == learning['operation'] and
                l['context'] == learning['context']):
                return l
        return None

    def _find_similar_failure(self, failure):
        """Find similar failure experiences"""
        for f in self.failures:
            if (f['operation'] == failure['operation'] and
                f['context'] == failure['context'] and
                f['error'] == failure['error']):
                return f
        return None

    def get_learnings_by_operation(self, operation):
        """Get all learnings for a specific operation"""
        return [l for l in self.learnings if l['operation'] == operation]

    def get_failures_by_operation(self, operation):
        """Get all failures for a specific operation"""
        return [f for f in self.failures if f['operation'] == operation]

    def get_unresolved_failures(self):
        """Get failures that haven't been resolved"""
        return [f for f in self.failures if not f.get('resolved', False)]

    def get_evolution_insights(self):
        """Generate insights for evolution planning"""
        insights = {
        ...
        insights['most_common_operations'] = dict(sorted(operations.items(), key=lambda x: x[1], reverse=True)[:10])
        ...
        insights['failure_patterns'] = dict(sorted(failure_ops.items(), key=lambda x: x[1], reverse=True)[:10])

        # Generate improvement opportunities
        for failure in self.get_unresolved_failures()[:5]:
            insights['improvement_opportunities'].append({
                'operation': failure['operation'],
                'error': failure['error'],
                'suggested_improvement': self._suggest_improvement(failure)
            })

        return insights

    def _suggest_improvement(self, failure):
        """Suggest improvements based on failure patterns"""
        similar_failures = [f for f in self.failures
                          if f['operation'] == failure['operation']
                          and f.get('resolved', False)]

        if similar_failures:
            solutions = [f['resolution'] for f in similar_failures if f['resolution']]
            if solutions:
                return f"Previously resolved with: {solutions[0]}"
        ...
        for func in self.functions:
            if func['complexity'] > 15:
                suggestions.append({
                    ...
                    'suggestion': f"Refactor {func['name']} - complexity {func['complexity']} is too high",
                    'priority': 'high'
                })
...
class SelfEvolutionManager:
    """Manager for self-evolution capabilities"""
    ...
    def record_failure(self, operation, context, error, solution_attempted=None, resolution=None):
        """Record a failure and potential resolution"""
    ...
    def _generate_evolution_plan(self, insights, suggestions):
        """Generate a comprehensive evolution plan"""
        plan = {
        ...
        for failure in insights['improvement_opportunities']:
            plan['priority_improvements'].append({
                'type': 'failure_resolution',
                'description': f"Resolve {failure['operation']} failures: {failure['error']}",
                'implementation': failure['suggested_improvement']
            })
        ...
        # Add performance improvements based on learnings
        if insights['total_learnings'] > 50:
            plan['priority_improvements'].append({
                'type': 'performance',
                'description': 'Optimize frequently used operations',
                'implementation': 'Implement caching and memoization for common patterns'
            })
    ...
    async def execute_evolution(self, evolution_plan):
        ...
        for improvement in evolution_plan['priority_improvements']:
            try:
                ...
                logger.error(f"Failed to execute improvement {improvement.get('target', improvement.get('type', 'unknown'))}: {e}")
        ...
        logger.info(f"✅ Self-evolution completed. New version: {self.current_version}")
        return results

    async def _refactor_function(self, improvement):
        """Refactor a function to reduce complexity"""
        logger.info(f"🔧 Refactoring {improvement['target']} - {improvement['suggestion']}")
        ...
        try:
            ...
            for i, line in enumerate(lines):
                if f'def {func_name}(' in line:
                    # Add refactoring comment above the function
    ...
    async def _enhance_error_handling(self, improvement):
    ...
    async def _implement_performance_optimization(self, improvement):
        ...
        try:
            ...
            if 'from functools import lru_cache' not in content and 'lru_cache' not in content:
                # Find the imports section and add caching import
                lines = content.split('\n')
                for i, line in enumerate(lines):
    ...
    async def _resolve_failure_patterns(self, improvement):
        ...
        logger.info(f"🔧 Resolving failure pattern: {improvement['description']}")
...
"""
    ...
    def get_evolution_status(self):
        ...
        return {
            ...
            'unresolved_failures': len(self.learning_db.get_unresolved_failures()),
...
""".strip()
        ...
        try:
            ...
            while not goal_achieved:
                ...
                if (
                    ...
                    # Schedule self-evolution based on learnings and failures
                    await self.schedule_self_evolution()
    ...
    async def execute_task(
        ...
        """Execute a task with cognitive capabilities and self-evolution tracking"""
        ...
        # Record task start for learning tracking
        task_context = {
            'type': task_type,
        ...
        try:
            ...
            if task_type == TaskType.SCAFFOLD.value:
                success, result = await self.execute_scaffold_task(task_details)
            elif (
                task_type == TaskType.BLUEPRINT_FILE.value
            ):  # FIX: Handle blueprinting task
                success, result = await self.execute_blueprint_file_task(task_details)
            elif task_type in [
                ...
                success, result = await self.execute_development_task(task_response, dev_strategy)
            elif (
                task_type == TaskType.TOOL_EXECUTION.value
            ):  # FIX: Handle tool execution task
                success, result = await self.execute_tool_task(task_details)
            elif (
                task_type == TaskType.PLAN_EPICS.value
                or task_type == TaskType.USER_STORY_REFINEMENT.value
            ):
                success, result = await self._execute_agile_task(task_response)
            elif task_type == TaskType.DEPLOYMENT.value:
                success, result = await self.execute_deployment_task(task_details, dev_strategy)
            elif task_type == TaskType.SECURITY_SCAN.value:
                success, result = await self.execute_security_scan_task(task_details, dev_strategy)
            elif task_type == TaskType.PERFORMANCE_OPTIMIZATION.value:
                success, result = await self.execute_performance_optimization_task(
                    task_details, dev_strategy
                )
            elif task_type == TaskType.COGNITIVE_ANALYSIS.value:
                success, result = (True, await self.execute_cognitive_analysis_task(task_details))
            elif task_type == TaskType.SELF_EVOLUTION.value:
                success, result = (True, await self.execute_self_evolution_task(task_details))
            elif task_type == TaskType.GIT_COMMIT.value:
                success, result = await self._execute_git_commit_task(task_details)
            else:
                logger.warning(f"Unknown or unhandled task type: {task_type}")
                success, result = (False, f"Unknown task type: {task_type}")

            # Record learning or failure
            task_context['end_time'] = time.time()
            task_context['duration'] = task_context['end_time'] - task_context['start_time']
            task_context['success'] = success

            if success:
                # Record successful learning
                self.evolution_manager.record_success(
                    operation=task_type,
                    context=task_context,
                    outcome=result,
                    improvement_notes=f"Task completed in {task_context['duration']:.2f}s using {dev_strategy}"
                )
            else:
                # Record failure
                self.evolution_manager.record_failure(
                    operation=task_type,
                    context=task_context,
                    error=result,
                    solution_attempted=dev_strategy
                )
            ...
            task_context['traceback'] = traceback.format_exc()
    ...
    async def execute_self_evolution_task(self, task_details: Dict) -> str:
        """Execute self-evolution based on learnings and failures"""
        evolution_type = (task_details or {}).get("evolution_type", "comprehensive")

        logger.info(f"🚀 Starting {evolution_type} self-evolution...")

        try:
            # Analyze evolution opportunities
            opportunities = self.evolution_manager.analyze_evolution_opportunities()

            if not opportunities['evolution_plan']['priority_improvements']:
                return "No evolution opportunities found. System is already optimized."

            # Execute the evolution plan
            evolution_results = await self.evolution_manager.execute_evolution(
                opportunities['evolution_plan']
            )

            success_count = len(evolution_results['success'])
            failed_count = len(evolution_results['failed'])

            result_message = f"""
Self-evolution completed:
✅ Successful improvements: {success_count}
❌ Failed improvements: {failed_count}
⏭️  Skipped: {len(evolution_results['skipped'])}

Current version: {self.evolution_manager.current_version}
Total learnings: {opportunities['insights']['total_learnings']}
Unresolved failures: {opportunities['insights']['unresolved_failures']}
"""

            # Update cognitive state to reflect evolution
            self.project_state.cognitive_state["evolution_count"] = (
                self.project_state.cognitive_state.get("evolution_count", 0) + 1
            )
            self.project_state.cognitive_state["last_evolution"] = time.time()
            self.project_state.cognitive_state["current_mode"] = CognitiveState.EVOLUTIONARY.value

            # Record this evolution as a learning
            self.evolution_manager.record_success(
                operation="SELF_EVOLUTION",
                context={
                    'evolution_type': evolution_type,
                    'improvements_attempted': len(opportunities['evolution_plan']['priority_improvements']),
                    'success_count': success_count,
                    'failed_count': failed_count
                },
                outcome=f"Completed {evolution_type} evolution with {success_count} improvements",
                improvement_notes=f"Advanced system to version {self.evolution_manager.current_version}"
            )

            return result_message.strip()

        except Exception as e:
            error_msg = f"Self-evolution failed: {str(e)}"
            ...
            self.evolution_manager.record_failure(
                operation="SELF_EVOLUTION",
                context={'evolution_type': evolution_type, 'task_details': task_details},
                error=str(e)
            )

            return error_msg

    async def schedule_self_evolution(self):
        """Schedule self-evolution tasks based on accumulated learnings"""
        if not self.evolution_manager.evolution_enabled:
            return

        # Check if we have enough learnings to warrant evolution
        status = self.evolution_manager.get_evolution_status()
        if status['total_learnings'] < 10 and status['total_failures'] < 5:
            return  # Not enough data for meaningful evolution

        # Check if we have unresolved failures
        unresolved = self.evolution_manager.learning_db.get_unresolved_failures()
        if unresolved:
            # Add self-evolution task to address failures
            self.project_state.task_queue.append({
                "task_type": TaskType.SELF_EVOLUTION.value,
                "task_description": f"Self-evolve to resolve {len(unresolved)} failures and optimize based on {status['total_learnings']} learnings",
                "details": {
                    "evolution_type": "comprehensive",
                    "context": f"System has {status['total_learnings']} learnings and {status['unresolved_failures']} unresolved failures"
                }
            })
            logger.info(f"📅 Scheduled self-evolution task with {len(unresolved)} failures to resolve")
...
async def main():
    ...
    try:
        ...
        print("=" * 30)
        print("  1. Self-evolve based on learnings and failures")
        print("  2. Analyze current system and suggest improvements")
        print("  3. Skip self-evolution (continue normally)")

        evolution_choice = input("Select evolution option (1, 2, or 3): ").strip()

        if evolution_choice == "1":
            print("🚀 Starting self-evolution process...")
            opportunities = evolution_manager.analyze_evolution_opportunities()

            print(f"\n📊 Evolution Analysis:")
            print(f"  - Total learnings: {opportunities['insights']['total_learnings']}")
            print(f"  - Total failures: {opportunities['insights']['total_failures']}")
            print(f"  - Unresolved failures: {opportunities['insights']['unresolved_failures']}")

            if opportunities['insights']['improvement_opportunities']:
                print(f"  - Priority improvements: {len(opportunities['evolution_plan']['priority_improvements'])}")

            proceed = input("\nProceed with self-evolution? (y/n): ").strip().lower()
            if proceed == 'y':
                await evolution_manager.execute_evolution(opportunities['evolution_plan'])
                print("✅ Self-evolution completed!")
            else:
                print("⏭️  Skipping self-evolution")

        elif evolution_choice == "2":
            print("🔍 Analyzing system for improvements...")
            opportunities = evolution_manager.analyze_evolution_opportunities()

            print(f"\n📊 System Analysis:")
            print(f"  - Current version: {evolution_manager.current_version}")
            print(f"  - Total learnings: {opportunities['insights']['total_learnings']}")
            print(f"  - Total failures: {opportunities['insights']['total_failures']}")

            if opportunities['source_analysis']:
                print(f"  - Lines of code: {opportunities['source_analysis']['total_lines']}")
                print(f"  - Functions: {opportunities['source_analysis']['functions']}")
                print(f"  - Classes: {opportunities['source_analysis']['classes']}")

            print(f"\n💡 Suggestions found: {len(opportunities['code_suggestions'])}")
            for suggestion in opportunities['code_suggestions'][:5]:
